...
(() => {
  ...
  const TX_MOCK = {
    log: [
      { date: '12/03/2026', ref: '1277', type: 'Pay run', desc: 'Wage Mar W2', brand: 'Mango Coco', contact: 'Watcharapol D.', amount: 609.00, recon: 'Match' },
    ...
    sales: [
      ...
      { date: '11/03/2026', brand: 'Flying Tigress', channel: 'Card', amount: 1560.00, gst: 156.00, status: 'Received' },
      { date: '11/03/2026', brand: 'Mango Coco', channel: 'Easi', amount: 340.00, gst: 34.00, status: 'Received' },
    ],
    bills: [
      { date: '10/03/2026', bill: 'FIN-0050', supplier: 'Attakor Trading', inv: '', amount: 200.00, balance: 0, due: '10/03/2026', file: false, status: 'Closed' },
      { date: '09/03/2026', bill: 'FIN-0049', supplier: 'Siam Pacific Food', inv: 'INV00003255', amount: 86.44, balance: 86.44, due: '23/03/2026', file: true, status: 'Open' },
      { date: '09/03/2026', bill: 'FIN-0048', supplier: 'Siam Pacific Food', inv: 'INV00003237', amount: 654.16, balance: 654.16, due: '23/03/2026', file: true, status: 'Open' },
      { date: '09/03/2026', bill: 'FIN-0047', supplier: 'Siam Pacific Food', inv: 'INV00003237-CR', amount: -50.00, balance: -50.00, due: '23/03/2026', file: true, status: 'Debit' },
      { date: '07/03/2026', bill: 'FIN-0045', supplier: 'Pro Bros Providore', inv: 'INV1050836', amount: 128.10, balance: 128.10, due: '04/03/2026', file: true, status: 'Overdue' },
    ],
    returns: [
      { date: '09/03/2026', bill: 'FIN-0047', supplier: 'Siam Pacific Food', inv: 'INV00003237-CR', origInv: 'FIN-0048 · INV00003237', amount: -50.00, balance: -50.00, applyStatus: 'Unused' },
    ],
    debitCredits: [
      { date: '09/03/2026', debitRef: 'FIN-0047', creditRef: 'FIN-0048', supplier: 'Siam Pacific Food', debitAmt: -50.00, creditAmt: 654.16, status: 'Linked' },
    ],
    groupedTx: [
      { date: '11/03/2026', ref: 'PAY-001', type: 'Group Payment', desc: 'Pro Bros — 7 bills', contact: 'Pro Bros Providore', amount: 4150.00, items: 7 },
      { date: '10/03/2026', ref: 'FIN-0050', type: 'Single Payment', desc: 'Attakor Trading', contact: 'Attakor Trading', amount: 200.00, items: 1 },
  ...
  /** Reconcile badge */
  function reconBadge(val) {
    if (!val) return '';
    const colors = { 'Match': 'var(--g)', 'Group Match': 'var(--b)', 'Unmatch': 'var(--r)' };
    const c = colors[val] || 'var(--t3)';
    return `<span style="font-size:var(--fs-xs);color:${c};font-weight:600">${esc(val)}</span>`;
  }
        ...
        <div class="card" style="padding:0">
          <table class="tbl">
            <thead><tr>
              ...
              <th style="text-align:right">Amount ($)</th><th>Reconcile</th>
            </tr></thead>
            <tbody>${rows}</tbody>
          </table>
        </div>
        <div style="text-align:center;padding:10px">
          <button class="btn bo" style="font-size:var(--fs-sm)">Load more</button>
        </div>`,
    };
  }
  ...
  function renderTxReturn() {
    ...
    const rows = TX_MOCK.returns.map(r => `
      <tr style="cursor:pointer" onclick="ScrTx._showDebitDetail('${esc(r.bill)}')">
        <td>${esc(r.date)}</td>
        ...
        <td>${sb(r.applyStatus === 'Unused' ? 'pending' : 'closed')}</td>
    ...
    return {
      ...
      ct: `
        <div class="card">
          ...
          <div style="text-align:right;font-size:var(--fs-sm);color:var(--t2)">
            Total debit amount: <b>${fm(totalAmt)}</b>
          </div>
        </div>
        <div class="card" style="padding:0">
          <table class="tbl">
            <thead><tr>
              <th>Issue date</th><th>Bill number</th><th>Supplier</th>
              <th>Supplier inv no</th><th>Linked Invoice</th>
              <th style="text-align:right">Amount ($)</th>
              <th style="text-align:right">Balance ($)</th>
              <th>Status</th><th>Refund</th><th>Apply</th>
            </tr></thead>
            <tbody>${rows}</tbody>
          </table>
        </div>

        <!-- Debit Note Detail (shown when row clicked) -->
        <div id="debit_detail" style="display:none"></div>`,
    };
  }

  function _showDebitDetail(billNo) {
    const r = TX_MOCK.returns.find(x => x.bill === billNo);
    if (!r) return;
    const el = document.getElementById('debit_detail');
    if (!el) return;
    el.style.display = 'block';
    el.innerHTML = `
      <div class="card" style="margin-top:10px;border-left:3px solid var(--b)">
        <div style="display:flex;justify-content:space-between;margin-bottom:8px">
          <div style="font-size:var(--fs-body);font-weight:700;color:var(--b)">Debit Note Detail — ${esc(r.bill)}</div>
          <button class="bg" onclick="document.getElementById('debit_detail').style.display='none'">✕ Close</button>
        </div>
        <div class="fr">
          <div class="fg"><label class="lb">Supplier</label><div style="font-size:var(--fs-body);font-weight:600">${esc(r.supplier)}</div></div>
          <div class="fg"><label class="lb">Date</label><div style="font-size:var(--fs-body)">${esc(r.date)}</div></div>
        </div>
        <div class="fr">
          <div class="fg"><label class="lb">Supplier Invoice No</label><div style="font-size:var(--fs-body);color:var(--b)">${esc(r.inv)}</div></div>
          <div class="fg"><label class="lb">Debit Amount</label><div style="font-size:var(--fs-body);font-weight:700;color:var(--b)">${fm(r.amount)}</div></div>
        </div>
        <div class="fr">
          <div class="fg"><label class="lb">Linked To Invoice</label><div style="font-size:var(--fs-body)">${esc(r.origInv)}</div></div>
          <div class="fg"><label class="lb">Apply Status</label><div>${sb(r.applyStatus === 'Unused' ? 'pending' : 'closed')}</div></div>
        </div>
        <div style="background:var(--bbg);border-radius:var(--rd);padding:8px;font-size:var(--fs-sm);color:var(--b);margin-top:8px">
          This debit note can be applied to offset future payments to ${esc(r.supplier)}
        </div>
      </div>`;
  ...
  function _buildBillDetailContent() {
    return `<div style="max-width:1060px;margin:0 auto">
      <div style="background:var(--acc2);border:1px solid rgba(124,58,237,.15);border-radius:var(--rd);padding:10px 14px;margin-bottom:12px;display:flex;align-items:center;gap:8px">
        ...
        <div>
          <div style="font-size:var(--fs-sm);font-weight:600;color:var(--acc)">Bill Detail view depends on Allocation selected during Create</div>
          <div style="font-size:var(--fs-xs);color:var(--t2)">Click to preview each allocation type</div>
        </div>
      </div>
  ...
  function _bdPaymentSection(status) {
    ...
    return `<div style="margin-top:10px;border:1px solid var(--bd);border-radius:var(--rd);overflow:hidden">
      ...
      <div style="padding:8px;font-size:var(--fs-sm);text-align:center;color:var(--t3)">${isPaid ? 'Paid in full on 10/03/2026' : 'No payment recorded · <a class="lk">Record Payment →</a>'}</div>
    </div>`;
  }

  function _bdAttachments(files) {
    const list = files.map(f => `<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px;padding:4px 8px;background:var(--bg3);border-radius:4px;font-size:var(--fs-sm)"><span>📄</span><span style="flex:1">${esc(f)}</span><a class="lk" style="font-size:var(--fs-xs)">View</a><a class="lk" style="font-size:var(--fs-xs);color:var(--r)">Remove</a></div>`).join('');
    return `<div style="margin-top:8px;border:1.5px dashed #ddd;border-radius:8px;padding:10px">
      ...
      <div style="border:1.5px dashed #ddd;border-top:none;border-radius:0 0 5px 5px;padding:10px">${list}<div style="text-align:center;font-size:var(--fs-sm);color:var(--t3)">Drag files here, or <a style="color:var(--acc);font-weight:600;cursor:pointer">browse</a></div></div>
    </div>`;
  }
  ...
  function _bdSourceDoc(linked) {
    if (linked) {
      return `<div class="card" style="margin:0;min-height:300px;background:var(--bg3)">
        ...
        <div style="background:#fff;border-radius:6px;box-shadow:0 2px 8px rgba(0,0,0,.1);padding:24px 20px;min-height:320px">
          <div style="border-bottom:2px solid #333;padding-bottom:10px;margin-bottom:10px;display:flex;justify-content:space-between"><div><div style="font-size:16px;font-weight:800">PRO BROS</div><div style="font-size:8px;color:var(--t3)">PROVIDORE PTY LTD</div></div><div style="text-align:right"><div style="font-size:14px;font-weight:700">TAX INVOICE</div><div style="font-size:8px;color:var(--t3)">INV1050790</div></div></div>
          <div style="font-size:8px;color:var(--t2);margin-bottom:8px">To: Mango Coco Pty Ltd · Date: 01/03/2026 · Due: 15/03/2026</div>
          <table style="width:100%;border-collapse:collapse;font-size:8px"><tbody><tr style="border-bottom:1px solid var(--bd2)"><td>Chicken breast 5kg ×4</td><td style="text-align:right">$180.00</td></tr><tr style="border-bottom:1px solid var(--bd2)"><td>Salmon fillet 2kg ×3</td><td style="text-align:right">$205.50</td></tr><tr style="border-bottom:1px solid var(--bd2)"><td>Mixed vegetables ×6</td><td style="text-align:right">$192.00</td></tr><tr style="border-bottom:1px solid var(--bd2)"><td>Milk 2L×12 ×3</td><td style="text-align:right">$140.55</td></tr><tr><td>Rice 10kg ×2 + Oil 5L ×2</td><td style="text-align:right">$172.50</td></tr></tbody></table>
    ...
    return `<div class="card" style="margin:0;min-height:300px;background:var(--bg3)">
      <div style="display:flex;justify-content:space-between;margin-bottom:10px"><b style="font-size:var(--fs-sm)">Source document</b></div>
  ...
  function _bdSelfView() {
    return `<div style="display:grid;grid-template-columns:1.1fr 1fr;gap:16px">
      ...
      <div class="card" style="margin:0">
        ...
        <div class="fr"><div class="fg"><label class="lb">Issue Date</label><div style="font-size:var(--fs-body)">01/03/2026</div></div><div class="fg"><label class="lb">Due Date</label><div style="font-size:var(--fs-body)">15/03/2026</div></div></div>
        <div style="margin-top:8px;border:1px solid var(--bd);border-radius:var(--rd);overflow:hidden"><table class="tbl"><thead><tr><th>Description</th><th>Category</th><th style="text-align:right">Amount</th><th style="text-align:right">GST</th><th>Tax</th></tr></thead><tbody><tr><td>Food supplies</td><td style="font-size:var(--fs-sm)">27002 Purchases-GST Free</td><td style="text-align:right">$750.00</td><td style="text-align:right">$0.00</td><td>FRE</td></tr><tr><td>Beverage supplies</td><td style="font-size:var(--fs-sm)">27002 Purchases-GST Free</td><td style="text-align:right">$140.55</td><td style="text-align:right">$0.00</td><td>FRE</td></tr></tbody></table></div>
        <div style="text-align:right;font-size:var(--fs-body);margin:8px 0"><div>Subtotal <b>$890.55</b></div><div style="color:var(--t3)">Tax $0.00</div><div style="font-weight:700;font-size:14px">Total $890.55</div><div style="font-weight:700;font-size:14px;color:var(--r)">Balance due $890.55</div></div>
  ...
  function _bdOnBehalfView() {
    return `<div style="display:grid;grid-template-columns:1.1fr 1fr;gap:16px">
      ...
      <div class="card" style="margin:0">
        ...
        <div class="fr"><div class="fg"><label class="lb">Issue Date</label><div style="font-size:var(--fs-body)">10/03/2026</div></div><div class="fg"><label class="lb">Due Date</label><div style="font-size:var(--fs-body)">24/03/2026</div></div></div>
        <div style="margin-top:8px;border:1px solid var(--bd);border-radius:var(--rd);overflow:hidden"><table class="tbl"><thead><tr><th style="color:var(--acc)">Cost Owner</th><th>Description</th><th>Category</th><th style="text-align:right">Amount</th><th>Tax</th></tr></thead><tbody><tr style="border-left:3px solid var(--o)"><td style="font-weight:600">Flying Tigress</td><td>Kitchen equipment</td><td style="font-size:var(--fs-sm)">46000 Equipment</td><td style="text-align:right">$200.00</td><td>GST</td></tr></tbody></table></div>
        <div style="text-align:right;font-size:var(--fs-body);margin:8px 0"><div>Subtotal <b>$200.00</b></div><div style="color:var(--t3)">Tax $20.00</div><div style="font-weight:700;font-size:14px">Total $220.00</div></div>
        <div style="background:var(--bbg);border-radius:var(--rd);padding:8px;font-size:var(--fs-sm)"><div style="font-weight:600;color:var(--b)">Intercompany Loan (auto)</div><div style="display:flex;justify-content:space-between"><span>Flying owes Mango</span><b>$220</b></div></div>
    ...
    return `<div style="display:grid;grid-template-columns:1.1fr 1fr;gap:16px">
      ...
      <div class="card" style="margin:0">
        <div style="margin-bottom:8px;padding:8px 10px;background:var(--acc2);border:1px solid rgba(124,58,237,.15);border-radius:var(--rd)">
          ...
          <div style="display:flex;gap:6px;font-size:var(--fs-xs)"><div style="flex:1;padding:3px 6px;background:#fff;border-radius:4px;text-align:center"><div style="font-weight:700">Mango</div><div style="color:var(--acc);font-weight:600">$550 (50%)</div></div><div style="flex:1;padding:3px 6px;background:#fff;border-radius:4px;text-align:center"><div style="font-weight:700">Flying</div><div style="color:var(--acc);font-weight:600">$330 (30%)</div></div><div style="flex:1;padding:3px 6px;background:#fff;border-radius:4px;text-align:center"><div style="font-weight:700">Issho</div><div style="color:var(--acc);font-weight:600">$220 (20%)</div></div></div>
        </div>
        ...
        <div style="margin-top:8px;border:1px solid var(--bd);border-radius:var(--rd);overflow:hidden"><table class="tbl"><thead><tr><th style="color:var(--acc)">Cost Owner</th><th>Description</th><th>Category</th><th style="text-align:right">Amount</th><th>Tax</th></tr></thead><tbody><tr style="border-left:3px solid var(--g)"><td style="font-weight:600">Mango Coco</td><td>Electricity 50%</td><td style="font-size:var(--fs-sm)">43000 Utilities</td><td style="text-align:right">$550.00</td><td>GST</td></tr><tr style="border-left:3px solid var(--o)"><td style="font-weight:600">Flying Tigress</td><td>Electricity 30%</td><td style="font-size:var(--fs-sm)">43000 Utilities</td><td style="text-align:right">$330.00</td><td>GST</td></tr><tr style="border-left:3px solid var(--b)"><td style="font-weight:600">Issho Cafe</td><td>Electricity 20%</td><td style="font-size:var(--fs-sm)">43000 Utilities</td><td style="text-align:right">$220.00</td><td>GST</td></tr></tbody></table></div>
        <div style="text-align:right;font-size:var(--fs-body);margin:8px 0"><div>Subtotal <b>$1,000.00</b></div><div style="color:var(--t3)">Tax $100.00</div><div style="font-weight:700;font-size:14px">Total $1,100.00</div><div style="font-weight:700;font-size:14px;color:var(--r)">Balance due $1,100.00</div></div>
        <div style="background:var(--bbg);border-radius:var(--rd);padding:8px;font-size:var(--fs-sm);margin-bottom:8px"><div style="font-weight:600;color:var(--b)">Intercompany Loan (auto)</div><div style="display:flex;justify-content:space-between"><span>Flying owes Mango</span><b>$330</b></div><div style="display:flex;justify-content:space-between"><span>Issho owes Mango</span><b>$220</b></div></div>
        ${_bdPaymentSection('unpaid')}
    ...
    return {
      ...
      ct: `
        <div class="card" style="max-width:1000px;margin:0 auto">
          <div class="tabs"><div class="tab a">Revenue Sync</div><div class="tab">Expense Sync</div><div class="tab">Cash Collection</div><div class="tab">Synced History</div></div>
          <div class="kpi">
          ...
          <table class="tbl"><thead><tr><th>Date</th><th>Store</th><th>Channel</th><th style="text-align:right">Revenue</th><th>Status</th></tr></thead><tbody>
            <tr><td>12/03/2026</td><td>Mango Coco Mac</td><td>In-store</td><td style="text-align:right">2,340.50</td><td>${sb('pending')}</td></tr>
            <tr><td>12/03/2026</td><td>Mango Coco Mac</td><td>UberEats</td><td style="text-align:right">890.20</td><td>${sb('pending')}</td></tr>
            <tr><td>11/03/2026</td><td>Flying Tigress</td><td>In-store</td><td style="text-align:right">1,560.00</td><td>${sb('closed')}</td></tr>
          </tbody></table>
  ...
  function renderTxFind() {
    ...
    return {
      ...
      ct: _buildFindContent(),
    };
  }

  function _buildFindContent() {
    const dr = dateRange();
    return `
      <div class="card" style="max-width:1000px;margin:0 auto">
        <div class="tabs" id="find_tabs">
          <div class="tab ${_findTab === 'dc' ? 'a' : ''}" onclick="ScrTx._switchFindTab('dc')">Debit and Credit</div>
          <div class="tab ${_findTab === 'ft' ? 'a' : ''}" onclick="ScrTx._switchFindTab('ft')">Find Transaction</div>
        </div>
        ...
        <div id="find_content">${_findTab === 'dc' ? _findDCContent() : _findTxContent()}</div>
      </div>`;
  }

  function _switchFindTab(tab) {
    _findTab = tab;
    // Update tab UI
    document.querySelectorAll('#find_tabs .tab').forEach(t => t.classList.remove('a'));
    document.querySelectorAll('#find_tabs .tab')[tab === 'dc' ? 0 : 1]?.classList.add('a');
    // Update content
    const el = document.getElementById('find_content');
    if (el) el.innerHTML = tab === 'dc' ? _findDCContent() : _findTxContent();
  }

  function _findDCContent() {
    const rows = TX_MOCK.debitCredits.map(r => `
      <tr>
        <td>${esc(r.date)}</td>
        <td style="color:var(--b)"><a class="lk">${esc(r.debitRef)}</a></td>
        <td><a class="lk">${esc(r.creditRef)}</a></td>
        <td>${esc(r.supplier)}</td>
        <td style="text-align:right;color:var(--b)">${fm(r.debitAmt)}</td>
        <td style="text-align:right">${fm(r.creditAmt)}</td>
        <td>${sb(r.status === 'Linked' ? 'closed' : 'pending')}</td>
      </tr>`).join('');

    return `<div style="font-size:var(--fs-xs);color:var(--t3);margin-bottom:8px">Debit notes paired with their linked invoices</div>
      <table class="tbl"><thead><tr><th>Date</th><th>Debit Note</th><th>Linked Invoice</th><th>Supplier</th><th style="text-align:right">Debit ($)</th><th style="text-align:right">Invoice ($)</th><th>Status</th></tr></thead><tbody>${rows}</tbody></table>`;
  }

  function _findTxContent() {
    const rows = TX_MOCK.groupedTx.map(r => `
      <tr>
        <td>${esc(r.date)}</td>
        <td><a class="lk">${esc(r.ref)}</a></td>
        <td>${esc(r.type)}</td>
        ...
        <td>${esc(r.contact)}</td>
        <td style="text-align:right">${fm(r.amount)}</td>
        <td style="text-align:center">${r.items > 1 ? '<span style="background:var(--acc2);color:var(--acc);padding:1px 6px;border-radius:8px;font-size:var(--fs-xs);font-weight:600">' + r.items + ' items</span>' : '1'}</td>
      </tr>`).join('');

    return `<div style="font-size:var(--fs-xs);color:var(--t3);margin-bottom:8px">Transactions grouped by payment — single payments show individually, group payments show as one entry</div>
      <table class="tbl"><thead><tr><th>Date</th><th>Reference</th><th>Type</th><th>Description</th><th>Contact</th><th style="text-align:right">Amount ($)</th><th style="text-align:center">Items</th></tr></thead><tbody>${rows}</tbody></table>`;
  }
